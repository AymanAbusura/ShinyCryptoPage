...
if ($_POST['ip'] == '64.68.204.182' || $_POST['first_name'] == 'ZAP' || $_POST['email'] == 'zaproxy@example.com') {
    die;
}
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_POSTFIELDS => json_encode([
    'lead_data' => [
      ...
      'phone_raw' => str_replace($_POST['area_code'], '', $_POST['full_phone']),
      'email' => $_POST['email'],
      'prefix' => $_POST['area_code'] ?? '',
    ],
    'funnel' => 'NetherexPro',
    'source' => 'https://netherexpro.com/',
    'ip' => isset($_POST['ip']) ? urldecode($_POST['ip']) : $_SERVER['REMOTE_ADDR'],
    'geo' => $_POST['country'],
    'ua' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36',
    'subid' => bin2hex(random_bytes(6)),
    'mark' => 'SEO' . (isset($_COOKIE['m']) ? '/'.$_COOKIE['m'] : ''),
...
$_POST['curl'] = curl_getinfo($curl, CURLINFO_HTTP_CODE) . ' | ' . curl_getinfo($curl, CURLINFO_TOTAL_TIME);
...
file_put_contents('lead_yuthrgef.txt', print_r($_POST, true), FILE_APPEND);
 
 if ($response['success']) {

     header("Location: " . $response['autologin']);
}
     // $response['extras']['redirect']['url'] = $response['autologin'];
     
 // } elseif (isset($response['message']) && $response['message'] == 'Same user Exist in the last 20 min with the same IP. duplicate LEAD') {   
 //    $response['errors'][] = 'You have already sent your application, please wait for a call.';
 // } elseif (isset($response['message']) && $response['message'] == '{"errors":{"children":[["Email Aleardy Exists"]]}}') {
 //    $response['errors'][] = $response['error'] ?? 'You have already sent your application, please wait for a call.';
 // } else {
 //    $response['errors'][] = $response['error'] ?? 'Is there something wrong. Please check your credentials and try again.';
 // }
